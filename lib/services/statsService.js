...
var getStatSummary = function(options) {
  var aggregate = matches => {
    ...
    const lblAMostD = 'Most Deaths';
    const lblTopGpm = 'Top GPM';
    const lblTopXpm = 'Top XPM';
    ...
    topStats[lblAMostD] = { value: 0, match: null };
    topStats[lblTopGpm] = { value: 0, match: null };
    topStats[lblTopXpm] = { value: 0, match: null };
    ...
    matches.forEach(m => {
      ...
      const kda = (kills + assists) / (deaths > 0 ? deaths : 1);
      ...
      if (deaths > topStats[lblAMostD].value) {
        topStats[lblAMostD].value = deaths;
        topStats[lblAMostD].displayValue = deaths;
        topStats[lblAMostD].match = matchId;
      }
      if (m.players.goldPerMin > topStats[lblTopGpm].value) {
        topStats[lblTopGpm].value = m.players.goldPerMin;
        topStats[lblTopGpm].displayValue = m.players.goldPerMin.toFixed(2);
        topStats[lblTopGpm].match = matchId;
      }
      if (m.players.xpPerMin > topStats[lblTopXpm].value) {
        topStats[lblTopXpm].value = m.players.xpPerMin;
